...
public class SelectTest extends TestBase {
    ...
    void select1() {
        ...
        //Partitioning column "f1" cannot be restricted because the preceding column ("id") is either not restricted or is restricted by a non-EQ relation
        cql = "select age, f2 " + //
                "from users where f1 = 10";
        tryPrintResultSet();
        ...
        //The token function arguments must be in the partition key order: ColumnDefinition{name=id, type=org.apache.cassandra.db.marshal.Int32Type, kind=PARTITION_KEY, componentIndex=0, indexName=null, indexType=null},ColumnDefinition{name=f1, type=org.apache.cassandra.db.marshal.Int32Type, kind=PARTITION_KEY, componentIndex=1, indexName=null, indexType=null}
        cql = "select age, f2 " + //
        ...
        //PRIMARY KEY column "f3" cannot be restricted (preceding column "ColumnDefinition{name=age}" is either not restricted or by a non-EQ relation)
        cql = "select age, f2 " + //
