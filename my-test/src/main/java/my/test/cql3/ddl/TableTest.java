...
public class TableTest extends TestBase {
    ...
    public void test_RawStatement_prepare() throws Exception {
        ...
        // 表名不能超过48个字符
        tryExecute("CREATE TABLE IF NOT EXISTS " + tableName
                + "toolonglonglonglonglonglonglonglonglonglonglong ( block_id uuid)");
        ...
        // 错误是:
        // COMPACT STORAGE with composite PRIMARY KEY allows no more than one column not part of the PRIMARY KEY (got:
        // f2, f1)
        tryExecute("CREATE TABLE IF NOT EXISTS " + tableName //
                + " ( block_id uuid, breed text, short_hair boolean, f1 text, f2 text," //
                + "PRIMARY KEY ((block_id, breed), short_hair)) WITH COMPACT STORAGE");
        ...
        tryExecute("CREATE TABLE IF NOT EXISTS " + tableName
                + " ( block_id uuid, breed text, color text, short_hair boolean,"
        ...
        tryExecute("CREATE TABLE IF NOT EXISTS " + tableName
                + " ( block_id uuid, breed text, color text, short_hair boolean," + "PRIMARY KEY (unknownField))");
        ...
        tryExecute("CREATE TABLE IF NOT EXISTS " + tableName
                + " ( block_id uuid, breed text, color text, short_hair boolean," + "PRIMARY KEY (block_id))");

        tryExecute("CREATE TABLE IF NOT EXISTS " + tableName
                + " ( block_id uuid, breed text, color text, short_hair counter," + "PRIMARY KEY (block_id))");
    }
    ...
    public void test_CFPropDefs_validate() throws Exception {
        ...
        // 有效的属性包括下面这些:
        // [read_repair_chance, bloom_filter_fp_chance, min_index_interval, speculative_retry, gc_grace_seconds,
        // dclocal_read_repair_chance, compaction,
        // max_index_interval, memtable_flush_period_in_ms, default_time_to_live, comment, compression, caching]
        tryExecute("CREATE TABLE IF NOT EXISTS " + tableName
                + " ( block_id uuid ) WITH min_threshold=2 AND index_interval=1");
        ...
        cql = "CREATE TABLE IF NOT EXISTS "
                + tableName
                + " (block_id uuid PRIMARY KEY, species text)"
                + //
    ...
    public void test_CreateTableStatement_applyPropertiesTo() throws Exception {
        // execute("CREATE TABLE IF NOT EXISTS " + tableName +
        // " ( block_id uuid PRIMARY KEY, breed text, emails set<text>)");
        ...
        tryExecute("CREATE TABLE IF NOT EXISTS "
                + tableName //
                + " ( block_id uuid, breed text, short_hair boolean, f1 text, f2 int, f3 text, f4 int static, " //
