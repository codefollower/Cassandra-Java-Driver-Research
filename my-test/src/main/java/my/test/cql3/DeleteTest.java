...
public class DeleteTest extends TestBase {
    ...
    void testRelation() throws Exception {
        ...
        cql = "SELECT f1 FROM " + tableName + " WHERE token(id)>1";
        printResultSet();
        ...
        cql = "DELETE f1 FROM " + tableName + " USING TIMESTAMP ? WHERE id=? AND f2=1 AND f3=2";
        PreparedStatement statement2 = session.prepare(cql);
        BoundStatement boundStatement2 = new BoundStatement(statement2);
        session.execute(boundStatement2.bind(10000L, 1));

        cql = "DELETE f1 FROM " + tableName + " USING TIMESTAMP :ts WHERE id=:id AND f2=1 AND f3=2";
        PreparedStatement statement3 = session.prepare(cql);
        BoundStatement boundStatement3 = new BoundStatement(statement3);
        boundStatement3.setInt("id", 1);
        boundStatement3.setLong("ts", 10000L);
        session.execute(boundStatement3);

        cql = "DELETE f1 FROM " + tableName + " WHERE (id, f2)>(1, 1)";
        tryExecute();

        cql = "DELETE f1 FROM " + tableName + " WHERE id=1 IF f1='ab'";
        tryExecute();

        cql = "DELETE f1 FROM " + tableName + " WHERE id=1";
        cql = "DELETE f1 FROM " + tableName + " WHERE token(id)>1";
        tryExecute();
        cql = "DELETE f1 FROM " + tableName + " WHERE token(id)>token(1)";
        tryExecute();
        cql = "DELETE f1 FROM " + tableName + " WHERE token(id,f2)>token(1, 3)";
        tryExecute();
    }
    ...
    void delete2() throws Exception {
        ...
        tryExecute();
        cql = "DELETE FROM " + tableName + " WHERE id=1 and f2=3";
        tryExecute();

        cql = "DELETE f1 FROM " + tableName + " WHERE id=1 and f2=3";
        tryExecute();
        ...
        // 对应org.apache.cassandra.cql3.statements.DeleteStatement.updateForKey(ByteBuffer, ColumnNameBuilder,
        // UpdateParameters)
        // 的if (isRange)
        execute("DELETE FROM " + tableName + " WHERE id=1 AND f1='abbc'");
